        ...
        <div class="mx-auto max-w-6xl px-4 py-20 sm:px-6 lg:px-8 lg:py-28">
            <div data-reveal class="max-w-2xl">
                ...
                <h2 class="mt-3 text-section text-balance text-fg">
                    {{ __('It reads the measurements, and') }}
                    <span style="color: var(--app-ai)">{{ __('invents nothing') }}</span>.
                </h2>

                <p class="mt-5 text-body-lg text-fg-muted">
                    {{ __('An AI that guesses at a cause is worse than no AI at all, because at three in the morning you act on it. So the limits are drawn first: it cites only the checks, regions and metrics it can see, it writes the evidence both for and against, and where it does not know it says so.') }}
                </p>
            </div>
            ...
            <div class="mt-14 grid gap-10 lg:grid-cols-2 lg:gap-16">
                ...
                <div data-reveal style="--reveal-index: 1; border-color: var(--app-ai-soft); background-color: var(--app-ai-wash)" class="rounded-lg border p-6 lg:p-8">
                    <h3 class="text-title-lg text-fg">{{ __('What it cannot see') }}</h3>
                    ...
                    <p class="mt-6 border-t pt-5 text-body-md text-fg-muted" style="border-color: var(--app-ai-soft)">
                        {{ __('Uptizm watches your endpoints from the outside, and that is the whole of what it knows. So it can tell you the queue crossed its bound three checks before the first 503. It cannot tell you a deploy caused it, and it will not pretend otherwise.') }}
                    </p>
                </div>
                ...
                <div data-reveal style="--reveal-index: 2">
                    <h3 class="text-title-lg text-fg">{{ __('You set what it is allowed to do') }}</h3>
                    ...
                    <p class="mt-7 text-body-md text-fg-muted">
                        {{ __('Its suggestions are advice. There is no button that lets it touch your infrastructure, because it has no way in.') }}
                    </p>
                </div>
            </div>
            ...
            <p data-reveal style="--reveal-index: 3" class="mt-12 border-t border-border pt-8 text-body-lg text-fg-muted">
                {{ __('And the AI is in the plan, not billed per incident. Pay-per-use AI sends its biggest invoice on your worst day, and a feature you hesitate to let run is not a feature.') }}
            </p>
        </div>
    </section>
@endif
