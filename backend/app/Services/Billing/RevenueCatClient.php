 ...
 * only place the store rail's truth enters the application
 * ({@see SyncRevenueCatEntitlement} is its only caller today).
 *
 ...
 *
 * A per-call timeout sized against a wall is wrong the moment anything retries:
 * this repo shipped a 75-second AI call under a 90-second wall, and the retry it
 * had forgotten about made 150. So one budget covers every attempt, a retry
 * only starts if enough of the budget is left to be worth starting, and the
 * arithmetic that matters holds by construction:
 *
 *   one authoritative read <= `revenuecat.operation_budget_seconds` (10)
 *   the sync job            <= its own `$timeout` (55)
 *   the Horizon worker      <= `supervisor-1`'s `timeout` (60)
 *
...
class RevenueCatClient
{
     ...
     * deployment choice; `revenuecat.base_url` overrides it so a test can point
     * the client at a local listener.
     */
    public const string DEFAULT_BASE_URL = 'https://api.revenuecat.com/v1';
     ...
     */
    public const int MAXIMUM_ATTEMPTS = 3;

    /**
     * The whole-operation budget, in seconds, when config does not name one.
     */
    protected const int DEFAULT_OPERATION_BUDGET_SECONDS = 10;

    /**
     * The smallest slice of the budget worth starting a RETRY with. The first
     * attempt always gets whatever the budget holds; a retry that could only run
     * for a moment would spend the remainder of the budget to fail again.
     */
    protected const float MINIMUM_RETRY_SECONDS = 2.0;

    /**
     * How long to wait before a retry, in milliseconds. Spent from the same
     * budget as the attempts themselves.
     */
    protected const int RETRY_DELAY_MS = 200;

    /**
     * Read one subscriber's authoritative state.
     *
     * @param  string  $appUserId  the RevenueCat App User ID, which in this
     *                             application is the bare team id
    ...
    {
        $configured = config('revenuecat.base_url');
     ...
     * The secret API key.
     *
     * Deliberately fallback-free. A default would be either a secret in a public
     * repository or a value that authenticates as nobody, and the second one
     * fails as a 401 the caller would then have to be careful not to read as an
     * expired subscription. An unconfigured rail refuses to ask.
     */
    protected function apiKey(): string
    {
        $key = config('revenuecat.secret_api_key');
    ...
    {
        $configured = config('revenuecat.operation_budget_seconds');
