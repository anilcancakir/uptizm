...
class CheckPersistenceService
{
    ...
    public function persist(Monitor $monitor, CheckResult $result): void
    {
        ...
        //    cannot acquire the lock is a live duplicate: the current holder
        //    will persist, so this attempt bows out.
        $lock = Cache::lock($this->lockKey($monitor, $result), self::LOCK_TTL_SECONDS);
        if (! $lock->get()) {
            return;
        }
        ...
        try {
            ...
            // 3. Write the check, refresh the denorm columns, and freeze metric
            //    samples in a single transaction so a partial batch never lands.
            [$check, $samples] = $this->persistWithinTransaction($monitor, $result);

            // 4. Threshold/signal routing runs only after the check is durable so
            //    a failure here cannot corrupt the committed telemetry. The
            //    monitor is refreshed so the evaluator reads the persisted
            //    consecutive-fail counter, not a stale in-memory value.
    ...
    protected function persistWithinTransaction(Monitor $monitor, CheckResult $result): array
    {
        return DB::transaction(function () use ($monitor, $result): array {
            ...
            //    counter. Only a `down` result advances the streak; any other
            ...
            $monitor->forceFill([
                ...
                'consecutive_fails' => $isDown ? ($monitor->consecutive_fails ?? 0) + 1 : 0,
            ])->save();
     ...
     */
    protected function lockKey(Monitor $monitor, CheckResult $result): string
    {
