 ...
 *
 * ## Why not extend the Stripe handler
 *
 * The discipline is copied, the code is not. `StripeWebhookController` is a
 ...
 *
 * The id is claimed as `rc:<event id>`. RevenueCat and Stripe issue ids from
 * different systems into ONE unique column, and the namespace prefix is what
 * keeps a collision between them from turning a real store delivery into a
 * permanent no-op.
 *
 ...
 *     event permanently, so an unknown team, an ignored type and an unreadable
 *     body all answer 200 having done nothing: a retry cannot improve any of
 *     them, and burning the delivery loses the ones that follow it too.
 ...
 *     budgeted (`revenuecat.operation_budget_seconds` inside the job's timeout
 *     inside the worker's).
 */
class RevenueCatWebhookController extends Controller
{
     ...
     * silently trusted.
     */
    public const string SIGNATURE_HEADER = 'X-RevenueCat-Webhook-Signature';
     ...
     */
    protected const int SIGNATURE_TOLERANCE_SECONDS = 300;
     ...
     */
    protected const array ENTITLEMENT_EVENT_TYPES = [
        'INITIAL_PURCHASE',
        'RENEWAL',
        'CANCELLATION',
        'UNCANCELLATION',
     ...
     */
    protected const string PRODUCTION_ENVIRONMENT = 'PRODUCTION';

    /**
     * The environment a sandbox purchase arrives under, accepted only where the
     * deployment has opted in.
     */
    protected const string SANDBOX_ENVIRONMENT = 'SANDBOX';
    ...
    public function __invoke(Request $request): Response
    {
        ...
        DB::transaction(function () use ($event): void {
            if (! ProcessedWebhookEvent::recordIfNew('rc:'.$event['id'], (string) $event['type'])) {
                return;
            }
     ...
     *
     * The event field decides; `revenuecat.accept_sandbox` only widens what it is
     * allowed to say. An absent or unrecognised value is treated as not
     * production, which is the direction that cannot cost money: it is no
     * evidence of a real purchase.
     *
     * `is_string` and never a cast, for the reason the rate limiters in
     * `bootstrap/app.php` carry at length: `(string)` on an array is "Array to
     * string conversion", which `HandleExceptions` promotes to an ErrorException,
     * and a 500 here BURNS one of five deliveries. A signed sender is not a
     * well-formed one.
     *
    ...
    protected function isActionableEnvironment(array $event): bool
    {
        ...
        return $environment === self::SANDBOX_ENVIRONMENT
            && (bool) config('revenuecat.accept_sandbox', false);
    }
