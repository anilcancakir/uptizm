...
- [ ] 9. Eksik trans() Cevirileri Ekle

  **What to do**:
  - `lib/resources/views/incidents/incident_show_view.dart`: 'Edit' (L299), 'Delete' (L318), 'Post Update' (L685) → `trans('common.edit')`, `trans('common.delete')`, `trans('incidents.post_update')`
  - `lib/resources/views/dashboard/dashboard_view.dart`: fallback 'there' (L19) → `trans('dashboard.greeting_fallback')`
  - Announcement view'larindaki placeholder text'ler → `trans()` ile sar
  - Ceviri dosyalarina (localization config) ilgili key'leri ekle
  - `/scan-translations` skill'ini kullanarak tum eksik cevirileri tara

  **Must NOT do**:
  - className string'lerini trans() ile sarma
  - Route isimlerini trans() ile sarma
  - Teknik identifier'lari trans() ile sarma

  **Recommended Agent Profile**:
  - **Category**: `unspecified-low`
    - Reason: Birden fazla dosyada string degisikligi
  - **Skills**: [`magic-framework`, `scan-translations`]
    - `magic-framework`: trans() facade API
    - `scan-translations`: Eksik cevirileri otomatik tespit
  ...
  **References**:
  - `lib/config/localization.dart` — Ceviri dosyasi yapisi
  - Mevcut `trans()` kullanim ornekleri icin herhangi bir view dosyasi

  **Acceptance Criteria**:
  - [ ] `/scan-translations` → 0 eksik ceviri (veya sadece bilerek haric tutulanlar)
  - [ ] Hardcoded 'Edit', 'Delete', 'Post Update' string'leri trans() ile sarili
  - [ ] `flutter test` → ALL PASS

  **Commit**: YES
  - Message: `fix(i18n): wrap hardcoded user-facing strings with trans()`
  - Files: incident_show_view.dart, dashboard_view.dart, announcement views, localization files
  - Pre-commit: `flutter test`

---

- [ ] 10. Enum Default Value Safety

  **What to do**:
  - `lib/app/enums/monitor_status.dart`: `fromValue` `orElse` callback'ini incele
  - Eger `orElse: () => MonitorStatus.active` ise → `orElse: () => MonitorStatus.unknown` veya null dondur
  - Bozuk/bilinmeyen veri geldiginde aktif monitor olarak gosterilmesi veri bozulmasi riskidir
  - Benzer pattern tum enum dosyalarinda taranmali
  - RED: Test yaz — bilinmeyen deger ile `fromValue` cagirildiginda `active` yerine `unknown`/null dondugunu dogrula
  - GREEN: Default degeri degistir
  - REFACTOR: Tum enum'larda tutarli pattern uygula

  **Must NOT do**:
  - Enum degerlerini degistirme
  - Yeni enum degerleri ekleme (unknown haric)
  - fromValue disindaki methodlari degistirme

  **Recommended Agent Profile**:
  - **Category**: `quick`
    - Reason: Pattern arama + kucuk degisiklikler
  - **Skills**: [`magic-framework`]
    - `magic-framework`: Enum fromValue pattern'i
  ...
  **References**:
  - `lib/app/enums/monitor_status.dart` — fromValue methodu
  - `test/app/enums/` — Mevcut enum testleri

  **Acceptance Criteria**:
  - [ ] Bilinmeyen deger ile fromValue → `null` veya `unknown` (asla `active` degil)
  - [ ] `flutter test test/app/enums/` → ALL PASS

  **Commit**: YES
  - Message: `fix(enums): return null/unknown for unrecognized enum values instead of defaulting to active`
  - Files: `lib/app/enums/monitor_status.dart`, ilgili test dosyalari
  - Pre-commit: `flutter test`

---

- [ ] 11. Controller Error Handling & Logging Iyilestirmesi

  **What to do**:
  - `lib/app/controllers/auth_controller.dart`: Generic `setError('An unexpected error occurred')` → `Log.error()` ile gercek hatayi logla, kullaniciya genel mesaj goster
  - Tum controller'larda catch bloklarini incele: gercek exception loglanıyor mu?
  - Pattern: `} catch (e, stackTrace) { Log.error('Auth login failed', e, stackTrace); setError(trans('errors.unexpected')); }`
  - RED: Test yaz — hata durumunda Log.error cagirildigini dogrula (mock ile)
  - GREEN: Error handling'i guncelle
  - REFACTOR: Tum controller'larda tutarli pattern uygula

  **Must NOT do**:
  - Kullaniciya detayli hata mesaji gosterme (guvenlik riski)
  - Controller is mantigi degistirme
  - Yeni error handling mekanizmasi olusturma (Log facade yeterli)

  **Recommended Agent Profile**:
  - **Category**: `unspecified-low`
    - Reason: 11 controller dosyasinda error handling review
  - **Skills**: [`magic-framework`]
    - `magic-framework`: Log facade API, controller pattern'leri
  ...
  **References**:
  - `lib/app/controllers/auth_controller.dart` — Error handling ornegi
  - `lib/app/controllers/` — 11 controller dosyasi
  - Magic Framework skill: Log facade API

  **Acceptance Criteria**:
  - [ ] Tum catch bloklarinda `Log.error()` cagrisi mevcut
  - [ ] Kullaniciya gosterilen hata mesajlari generic (exception detayi yok)
  - [ ] `flutter test` → ALL PASS

  **Commit**: YES
  - Message: `fix(controllers): add proper error logging with Log facade in all catch blocks`
  - Files: 11 controller dosyasi
  - Pre-commit: `flutter test`

---

- [ ] 12. Monitor Load Null Flicker Fix

  **What to do**:
  - `lib/app/controllers/monitor_controller.dart`: `loadMonitor` basarisiz oldugunda `selectedMonitorNotifier.value = null` set ediyor
  - Bu UI flickering'e neden olur — kullanici mevcut monitoru gorurken birdenbire bos ekran
  - Cozum: Hata durumunda mevcut degeri koru, ayri bir error state goster
  - Pattern: `try { ... selectedMonitorNotifier.value = monitor; } catch (e) { Log.error(...); setError(...); /* selectedMonitorNotifier degistirme */ }`

  **Must NOT do**:
  - Monitor loading mantığını degistirme
  - Yeni state management mekanizmasi ekleme
  - selectedMonitorNotifier tipini degistirme

  **Recommended Agent Profile**:
  - **Category**: `quick`
    - Reason: Tek dosyada kucuk mantik degisikligi
  - **Skills**: [`magic-framework`]
    - `magic-framework`: Controller state management pattern'leri
  ...
  **References**:
  - `lib/app/controllers/monitor_controller.dart` — loadMonitor methodu

  **Acceptance Criteria**:
  - [ ] loadMonitor basarisiz oldugunda selectedMonitorNotifier.value degismiyor
  - [ ] Error state kullaniciya gosteriliyor
  - [ ] `flutter test test/app/controllers/monitor_controller_test.dart` → PASS

  **Commit**: YES
  - Message: `fix(controller): preserve current monitor on load failure to prevent UI flicker`
  - Files: `lib/app/controllers/monitor_controller.dart`, test dosyasi
  - Pre-commit: `flutter test`

---

- [ ] 13. Empty State Widget'lari Ekle

  **What to do**:
  - Monitor, Incident, Status Page index view'larina empty state widget'lari ekle
  - Veri yokken kullaniciya "Henuz [X] yok. Ilk [X]'inizi olusturun" mesaji goster
  - CTA butonu ile create view'ina yonlendir
  - Dark mode destegi ile (`dark:` variant'lar)
  - Her empty state icin ilustrasyon veya ikon ekle (Material Symbols Outlined)

  **Must NOT do**:
  - Index view'larin listleme mantığını degistirme
  - Yeni sayfa ekleme
  - API degisikligi yapma

  **Recommended Agent Profile**:
  - **Category**: `visual-engineering`
    - Reason: UI component tasarimi, dark mode, ikon secimi
  - **Skills**: [`wind-ui`, `flutter-design`, `mobile-app-design-mastery`]
    - `wind-ui`: WDiv, WText, WButton className pattern'leri
    - `flutter-design`: Spacing, typography, color scheme
    - `mobile-app-design-mastery`: Empty state UX best practices
  ...
  **References**:
  - `lib/resources/views/monitors/monitor_index_view.dart` — Mevcut liste yapisi
  - `lib/resources/views/incidents/` — Incident index view
  - `lib/resources/views/status_pages/` — Status page index view
  - Brand guide: Primary `#009E60`, dark mode required, Material Symbols Outlined

  **Acceptance Criteria**:
  - [ ] Bos liste durumunda empty state widget gorunuyor
  - [ ] Empty state'te CTA butonu var ve dogru sayfaya yonlendiriyor
  - [ ] Dark mode'da gorsel tutarlilik
  - [ ] `flutter test` → ALL PASS

  **Commit**: YES
  - Message: `feat(ui): add empty state widgets for monitor, incident, and status page lists`
  - Files: 3 index view dosyasi
  - Pre-commit: `flutter test`

---

- [ ] 14. Model Debug Log Temizligi
