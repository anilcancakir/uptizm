...
///
/// Mirrors `backend/app/Enums/PlanStatus.php` case for case. The eight cases are
...
/// pause primitive, and Stripe reports `paused` too (`STATUS_PAUSED` in the
/// installed `stripe/stripe-php` `Subscription`), which the backend leaves out
/// of its explicit status table so it lands here through the neutral fallback. A
/// rail without the concept simply never sends the case, whereas adding it later
...
/// `subscribed`, which the server derives from the tier plus `PlanStatus::grants()`.
/// A second definition here could disagree with the first, and the server's is
/// the one that gates.
enum PlanStatus {
  /// No plan lifecycle at all: a team no rail has ever charged, and the landing
...
/// Decodes the backend `plan_status` wire value into a [PlanStatus], falling
/// back to [PlanStatus.none] on an absent or unrecognised value.
///
...
/// backend words are snake_case (`past_due`) where Dart is camelCase, so a
/// `.name` comparison would silently drop that case into the fallback.
PlanStatus planStatusFromWire(String? raw) {
  return switch (raw) {
