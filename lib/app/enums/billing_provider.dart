/// Which rail granted the entitlement the team currently holds.
///
/// Mirrors `backend/app/Enums/BillingProvider.php` case for case. The
...
///
/// This enum answers "who is billing this team", and nothing more. It says
...
enum BillingProvider {
  /// Nobody is billing: a team no rail has ever charged, and the landing place
  ...
  /// The two mobile stores, which bill on our behalf and keep management of the
  /// purchase inside their own account surface.
  appStore,
  playStore,
...
/// Decodes the backend `provider` wire value into a [BillingProvider], falling
/// back to [BillingProvider.none] on an absent or unrecognised value.
///
/// The fallback is what lets a newer backend ship a sixth rail without
/// crashing an older client. It lands on [BillingProvider.none] rather than on
/// a real rail on purpose: reading an unknown provider as an existing one would
/// have the client attribute a grant to a rail that never made it.
///
/// The values are matched EXPLICITLY rather than against `.name`, because the
/// backend words are snake_case (`app_store`) where Dart is camelCase, so a
/// `.name` comparison would silently drop both store rails into the fallback.
BillingProvider billingProviderFromWire(String? raw) {
  return switch (raw) {
