...
class Monitor extends Model with HasTimestamps, InteractsWithPersistence {
  ...
  static Future<Monitor?> find(int id) async {
    ...
    final result = await InteractsWithPersistence.findById<Monitor>(id, Monitor.new);
    Log.debug('Monitor.find result: ${result != null ? "found (${result.name})" : "null"}');
    return result;
  }

  static Future<List<Monitor>> all() async {
    Log.debug('Monitor.all called');
    final result = await InteractsWithPersistence.allModels<Monitor>(Monitor.new);
