...
class Incident extends Model with HasTimestamps, InteractsWithPersistence {
  ...
  String get monitorName {
    ...
    // A type TEST, not a cast. This getter runs inside `build` on the incident
    // detail screen, and a hard cast throws on a wrong-typed field, so one odd
    // row in the `monitors` pivot took the whole screen down rather than
    // blanking a name. The same reasoning that swept 75 model casts to
    // `get<T>`; these read nested maps, so `get<T>` did not reach them.
    final Object? name = primary['name'];
  ...
  IncidentAi? get ai {
    ...
    return IncidentAi(
      ...
      confirmed: raw['confirmed'] is bool ? raw['confirmed'] as bool : null,
    );
  }
